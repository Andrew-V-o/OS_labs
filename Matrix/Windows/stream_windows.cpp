...
DWORD WINAPI ThreadMultiplyWorker(LPVOID lpParam) {
    ...
    while (true) {
        ...
        for (int i = iStart; i < iEnd; ++i) {
            for (int j = jStart; j < jEnd; ++j) {
                int sum = 0;
                for (int m = 0; m < n; ++m) 
                    sum += (*(data->inputA))[i][m] * (*(data->inputB))[m][j];
...
int main() {
    ...
    for (int i = 0; i < numThreads; ++i) {
        HANDLE h = CreateThread(NULL, 0, ThreadMultiplyWorker, &data, 0, NULL);
    ...
    cout << "Размер блока k: " << k << "\nВсего блоков: " << totalBlocks 
         ...
         << "\nМногопоточное время (Windows API): " << fixed << setprecision(6) << elapsed.count() << " сек" << endl;
